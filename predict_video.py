...
parser.add_argument("--output_video_path", type=str, default= r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/out1.mp4")
...
yolo_classes = r"/content/drive/MyDrive/Padle_Analytics/yolov3.txt"
yolo_weights = r"/content/drive/MyDrive/Padle_Analytics//yolov3.weights"
yolo_config = r"/content/drive/MyDrive/Padle_Analytics/yolov3.cfg"

if output_video_path == "":
    # output video in same path
    output_video_path = input_video_path.split('.')[0] + r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/out1.mp4"

# get video fps&video size
video = cv2.VideoCapture(r"/content/drive/MyDrive/Padle_Analytics/VideoInput/input.mp4")
...
m.load_weights("/content/model.h5")
...
# Define paths
game_video_path = r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/out1.mp4"
minimap_video_path = r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/minimap1.mp4"


# Open game video
game_video = cv2.VideoCapture(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/out1.mp4")
...
#fourcc = cv2.VideoWriter_fourcc(*"mp4v")
#output_video = cv2.VideoWriter(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/video_with_map1.mp4", fourcc, fps1, (output_width, output_height))

print("🛠️ Adding the mini-map...")

# Processing steps (Ensure mini-map is generated)
x, y = diff_xy(coords)
remove_outliers(x, y, coords)
coords = interpolation(coords)
create_top_view(court_detector, detection_model, coords, fps1)


# Load Mini-map Video
minimap_video = cv2.VideoCapture(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/minimap1.mp4")

#if not minimap_video.isOpened():
#    print("❌ Error: Could not open mini-map video! Trying conversion...")

    # Convert to H.264 if necessary
#    converted_minimap_path = r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/video_with_map_fixed1.mp4"
#    os.system(f'ffmpeg -i "{minimap_video_path}" -vcodec libx264 "{converted_minimap_path}"')

#    minimap_video = cv2.VideoCapture(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/video_with_map_fixed1.mp4")
    
#    if not minimap_video.isOpened():
#        print("❌ Error: Mini-map video could not be opened even after conversion.")
#        exit()
#    else:
#        print("✅ Successfully converted mini-map video.")

fps2 = int(minimap_video.get(cv2.CAP_PROP_FPS))
print("🗺️ Mini-map FPS:", fps2)
...
if bounce == 1:
  ...
  #if minimap == 1:
   # video = cv2.VideoCapture(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/video_with_map1.mp4")
  #else:
  ...
  output_video = cv2.VideoWriter(r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/final_video1.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps, (output_width, output_height))
  i = 0
  while True:
    ret, frame = video.read()
    if ret:
