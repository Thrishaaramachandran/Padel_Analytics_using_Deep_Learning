...
def create_top_view(court_detector, detection_model, xy, fps):
    ...
    coords = xy[:]  # Copy ball position coordinates
    ...
    output_path = r"/content/drive/MyDrive/Padle_Analytics/VideoOutput1/minimap1.mp4"
    out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (v_width, v_height))

    # Players' locations on the court
    ...
    for i, (feet_pos_1, feet_pos_2) in enumerate(zip(smoothed_1, smoothed_2)):
        frame = court.copy()  # Copy court frame for each iteration
        
        if feet_pos_1 is not None:
            frame = cv2.circle(frame, (int(feet_pos_1[0]), int(feet_pos_1[1])), 45, (255, 0, 0), -1)

        if feet_pos_2 is not None and feet_pos_2[0] is not None:
            frame = cv2.circle(frame, (int(feet_pos_2[0]), int(feet_pos_2[1])), 45, (255, 0, 0), -1)

        if i < len(coords):  # Ensure we don't exceed list index
            draw_ball_position(frame, court_detector, coords[i], i)

        out.write(frame)

    out.release()
    print(f"Top-view video saved at: {output_path}")
...
if __name__ == "__main__":
  ...
  video = cv2.VideoCapture('VideoInput/input.mp4')
  ...
  cap = cv2.VideoCapture('VideoInput/input.mp4')
  print('Cap FPS ', cap.get(cv2.CAP_PROP_FPS))
  #props
  fps, length, width, height = get_video_properties(cap)

  # Video writer
  out = cv2.VideoWriter('VideoOutput/out1.mp4',fourcc, fps, (width, height))
