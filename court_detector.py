  ...
  def track_court(self, frame):
        ...
        for line in lines:
            ...
            if len(new_points) < 50:
                print(f"Tracking failed: only {len(new_points)} points found.")
                if self.dist > 100:
                    print("Max distance reached, re-detecting court...")
                    return self.detect(frame)  # Avoid infinite recursion
                print("Court tracking failed, adding 5 pixels to dist")
                self.dist += 5
                return self.track_court(frame)  # Recursive call
